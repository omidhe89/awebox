...
from megawes_settings_modified import set_megawes_settings, set_path_generation_options, solve_mpc_step
...
# ----------------- create code-generated functions ----------------- #
...
# create simulator
simulator = sim.Simulation(trial, 'open_loop', mpc_sampling_time, options)

# ----------------- create code-generated simulator function ----------------- #
...
xf_scaled = trial.model.variables_dict['x'](simulator.F(x0=x0_scaled, p=u_scaled)['xf'])
xf_si = []
for name in list(mpc.trial.model.variables_dict['x'].keys()):
...
zf_scaled = dae.z(simulator.F(x0=x0_scaled, p=u_scaled)['zf'])
...
for name in list(mpc.trial.model.variables_dict['z'].keys()):
    xa_si.append(lambda_scaled[name] * scaling['z'][name])
xa_si = ca.vertcat(*xa_si)
...
# generate simulator function
F_sim = ca.Function('F_sim', [x0_si, u0_si], [xf_si, xa_si, xdot_si], ['x0', 'u0'], ['xf', 'xa', 'xdot'])

# ----------------- compile simulator functions ----------------- #

# compile simulator function
if compile_simulator:

    # Solver code/shared library file names
    filename1 = foldername + 'F_sim.c'
    filename2 = foldername + 'F_sim.so'

    # generate solver dependencies
    ts = time.time()
    F_sim.generate('F_sim.c')

    # Compile dependencies
    os.system("mv F_sim.c" + " " + filename1)
    os.system("gcc -fPIC -shared -O3 " + filename1 + " -o " + filename2)
    tc = time.time() - ts
    print("INFO:   Open-loop simulator compilation time: {} s".format(np.round(tc, 3)))
...
modified_options.fill_in_seed(options)
...
# TODO: generate integrator function
# TODO: F_int = ca.Function('F_int') # ... [x0_si, u0_si], [xf_si, xa_si, xdot_si], ['x0', 'u0'], ['xf', 'xa', 'xdot'])

# ----------------- create code-generated integrator ----------------- #
...
if compile_integrator:
    ...
    os.system("gcc -fPIC -shared -O3 " + filename1 + " -o " + filename2)
    tc = time.time() - ts
    print("INFO:   Integrator compilation time: {} s".format(np.round(tc, 3)))
