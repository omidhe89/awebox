...
N_loops = 5     # Number of loops per power cycle
N_nlp = 120     # Number of NLP discretization intervals

# Simulation parameters:
builtin_mpc = False     # Flag for additional built-in simulation
T_end = 100.0           # End time of simulation
ts = 0.1                # MPC sample time (simulation window of each MPC evaluation)
N_sim = int(T_end/ts)   # Number of MPC evaluations
N_dt = 50               # Number of time steps per MPC evaluation (equivalent to option: 'number_of_finite_elements')
dt = float(ts/N_dt)     # Size of simulation time step
...
options['model.system_bounds.theta.t_f'] = [1.0, 80.0]
...
trial.build()
trial.optimize()
trial.write_to_csv(trial_name)
#trial.plot('isometric')

#____________________________________________________________________
# Built-in closed loop simulation with MPC (default options)
if builtin_mpc:
    sim = awe.sim.Simulation(trial, 'closed_loop', ts, trial.options_seed)
    sim.run(N_sim)
    #sim.plot('isometric')
...
integrator = awe_integrators.rk4root(
    ...
    dae.dae,
    dae.rootfinder,
    {'tf': dt, 'number_of_finite_elements': N_dt})
...
# ____________________________________________________________________
# Run simulation with combined MPC/External forces
...
# Monitor time
time.strftime("--- Elapsed simulation time t=%M:%Ss ---", time.gmtime(time.time() - start_time))
